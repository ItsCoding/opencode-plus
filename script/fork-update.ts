...
export async function runForkUpdate(options: Options = {}) {
  const paths = options.paths ?? {
    root,
    binary: binaryPath(root, process.arch),
    installed,
  }
  ...
  await command("bun run --cwd packages/cli typecheck", { cwd: paths.root })
  await command("bun run --cwd packages/cli build -- --single", {
    cwd: paths.root,
    env: { OPENCODE_VERSION: release.tag.slice(1) },
  })
...
export function binaryPath(root: string, arch: string) {
  return path.join(root, "packages", "cli", "dist", `cli-darwin-${arch}`, "bin", "opencode")
}
